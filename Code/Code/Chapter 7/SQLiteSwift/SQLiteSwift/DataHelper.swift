...
class TeamDataHelper: DataHelperProtocol {
    ...
    static func find(id: Int64) throws -> T? {
        ...
        for item in  items {
            return TeamData(teamId: item[teamId] , city: item[city], nickName: item[nickName], abbreviation: item[abbreviation])
        }
    ...
    static func findAll() throws -> [T]? {
        ...
        for item in items {
            retArray.append(TeamData(teamId: item[teamId], city: item[city], nickName: item[nickName], abbreviation: item[abbreviation]))
        }
...
class PlayerDataHelper: DataHelperProtocol {
    ...
    static func find(id: Int64) throws -> T? {
        ...
        let items = DB.prepare(query)
        for item in  items {
            return PlayerData(playerId: item[playerId], firstName: item[firstName], lastName: item[lastName], number: item[number], teamId: item[teamId], position: Positions(rawValue: item[position]))
        }
    ...
    static func findAll() throws -> [T]? {
        ...
        let items = DB.prepare(table)
        for item in items {
            retArray.append(PlayerData(playerId: item[playerId], firstName: item[firstName], lastName: item[lastName], number: item[number], teamId: item[teamId], position: Positions(rawValue: item[position])))
        }
